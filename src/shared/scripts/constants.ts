...
export const getNextCollectionNumber = (items: any[] = []) => Math.max(0, ...items?.map(item => Number(item?.number || getAppCollectionIDNumber(item?.id) || 0)).filter(number => Number.isInteger(number))) + 1;
